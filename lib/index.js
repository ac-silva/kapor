...
const addFunctions = (array) => {
    array.get = function(letter, row){
        if(arguments.length === 1) {
            let cell = arguments[0];
            cell     = cell.split(/(\d+)/).filter(Boolean);
            letter   = cell[0];
            row      = cell[1];
        }
